...
export default function RegretAnalysis({ decisions }) {
  ...
  if (stats.regretCount === 0) return null;
            ...
            {stats.top.map(([reason, count]) => {
              ...
              return (
                <div key={reason} className="flex items-center gap-2.5">
                  <Badge className="rounded-lg text-xs bg-[#f3e2d4] text-[#7a4a2d] border-[#e5d5c8] shrink-0">
                    {reason}
                  </Badge>
                  ...
                  <span className="text-[11px] text-[#a09080] shrink-0 tabular-nums">
                    {count} 次
                  </span>
                </div>
              );
